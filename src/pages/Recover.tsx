...
export function Recover() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const send = await api.recoverSendCode(email);
      setCode(send.code);
      setUserId(send.userId);
      if (send) {
      ...
      toast.error("Nenhum usuário foi encontrado");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSubmitpassword = async (e: React.FormEvent) => {
    ...
    try {
      if (email != "") {
        const response = await api.recoverPassword(password, userId);
        if (!response) {
          return;
        }
